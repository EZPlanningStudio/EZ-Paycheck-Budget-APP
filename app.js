...
function init() {
    document.title = `EZ Paycheck Budget App v${APP_VERSION}`;
    ...
    const savedSection = localStorage.getItem("ezPaycheckActiveSection");
    const hasSeenQuickStart = localStorage.getItem("ezPaycheckHasSeenQuickStart");

    if (!hasSeenQuickStart) {
        showSection("quickstart");
        localStorage.setItem("ezPaycheckHasSeenQuickStart", "true");
...
function setRainbowTitle(text, secondaryWord = null) {
    ...
    const activeSection = localStorage.getItem("ezPaycheckActiveSection") || "list";
...
function closeAddBillModal() {
    ...
    resetForm();
    if (localStorage.getItem("ezPaycheckActiveSection") === "add") {
        localStorage.setItem("ezPaycheckActiveSection", "list");
    }
}
...
function showSection(section) {
    ...
    localStorage.setItem("ezPaycheckActiveSection", section);
...
function openPaycheckInfoModal() {
    if (localStorage.getItem("ezPaycheckPaycheckSeen") === "true") return;
    setTimeout(() => {
...
function closePaycheckInfoModal(dontShow = false) {
    if (dontShow) localStorage.setItem("ezPaycheckPaycheckSeen", "true");
    document.getElementById("paycheckInfoModal").classList.remove("active");
}

function openCalendarInfoModal() {
    if (localStorage.getItem("ezPaycheckCalendarSeen") === "true") return;
    setTimeout(() => {
...
function closeCalendarInfoModal(dontShow = false) {
    if (dontShow) localStorage.setItem("ezPaycheckCalendarSeen", "true");
...
function openTransactionListInfoModal() {
    if (localStorage.getItem("ezPaycheckTransactionListSeen") === "true") return;
    setTimeout(() => {
...
function closeTransactionListInfoModal(dontShow = false) {
    if (dontShow) localStorage.setItem("ezPaycheckTransactionListSeen", "true");
...
function openMonthlyInfoModal() {
    if (localStorage.getItem("ezPaycheckMonthlySeen") === "true") return;
    setTimeout(() => {
...
function closeMonthlyInfoModal(dontShow = false) {
    if (dontShow) localStorage.setItem("ezPaycheckMonthlySeen", "true");
    document.getElementById("monthlyInfoModal").classList.remove("active");
}

function openYearlyInfoModal() {
    if (localStorage.getItem("ezPaycheckYearlySeen") === "true") return;
    setTimeout(() => {
...
function closeYearlyInfoModal(dontShow = false) {
    if (dontShow) localStorage.setItem("ezPaycheckYearlySeen", "true");
...
function isActivated() {
    return localStorage.getItem("ezPaycheckActivated") === "true";
}

function getHiddenMenuItems() {
    try {
        const parsed = JSON.parse(localStorage.getItem("ezPaycheckHiddenMenuItems") || "[]");
...
function hideMenuItem(section) {
    ...
    localStorage.setItem("ezPaycheckHiddenMenuItems", JSON.stringify(hidden));
    applyHiddenMenuItems();
    if (localStorage.getItem("ezPaycheckActiveSection") === section) showSection("list");
    ...
    if (section === "list") {
        ...
        const leftHtml = totalBase > 0
            ? (amountLeft >= -0.005
                ...
                : `<span style="color:var(--red);font-size:12px;white-space:nowrap;"><span class="help-icon" data-help-title="Overspent — How it works" data-help="This means your savings and cash/debit expenses added up to more than your rollover plus income received.&lt;br&gt;&lt;br&gt;Expenses paid with a credit card are &lt;strong&gt;not counted&lt;/strong&gt; here — they appear in your category totals but don't affect your available cash.&lt;br&gt;&lt;br&gt;Formula: Rollover + Income received − Savings − Cash expenses" style="cursor:pointer;margin-right:4px;">📊</span>Overspent by <strong>${formatMoney(Math.abs(amountLeft))}</strong></span>`)
            : "";
...
function renderCalendar() {
    ...
    renderCalSummaryCards();
    if (localStorage.getItem("ezPaycheckActiveSection") === "calendar") {
        renderCalProgressBar();
    }
                ...
                <div class="mi-budget-top-row">
                    ...
                    <div class="mi-budget-input-row">
                        ...
                        <input class="mi-budget-input" type="number" placeholder="0.00"
                            style="color:${(isPaycheckKey(_mk) ? data.paycheckBudgets : data.monthlyBudgets)?.[_mk]?.rollover > 0 ? 'var(--peach-text)' : 'var(--muted)'};border-color:var(--peach);"
                            value="${(isPaycheckKey(_mk) ? data.paycheckBudgets : data.monthlyBudgets)?.[_mk]?.rollover > 0 ? parseFloat((isPaycheckKey(_mk) ? data.paycheckBudgets : data.monthlyBudgets)[_mk].rollover).toFixed(2) : ''}"
...
function saveRollover(input, mk) {
    const val = parseFloat(input.value) || 0;
    const store = isPaycheckKey(mk) ? "paycheckBudgets" : "monthlyBudgets";
    if (!data[store]) data[store] = {};
    if (!data[store][mk]) data[store][mk] = {};
    if (val === 0) {
        delete data[store][mk].rollover;
    } else {
        data[store][mk].rollover = val;
    }
    saveData();
}
...
const BACKUP_DB_NAME = "ezPaycheckBackupDB";
...
async function showWelcomeBackupReminder() {
    ...
    const hasOpenedBefore = localStorage.getItem("ezPaycheckHasOpenedBefore");
    const shownThisSession = sessionStorage.getItem("welcomeBackupShownThisSession");

    if (!hasOpenedBefore) {
        localStorage.setItem("ezPaycheckHasOpenedBefore", "true");
        return;
    }
...
async function exportCsv() {
    ...
    const fileName = "ez-paycheck-data.csv";
