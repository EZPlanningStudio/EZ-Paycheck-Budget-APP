...
function renderPaycheck() {
    ...
    const rolloverInput = el.querySelector(".mi-cat-card:first-child .mi-budget-input");
    if (rolloverInput) {
        rolloverInput.setAttribute("oninput", `savePaycheckRollover(this, '${pc.id}')`);
    }

    applyPaycheckTextOverrides(el);
}

function applyPaycheckTextOverrides(el) {
    ...
    if (rolloverLabel) {
        ...
        if (helpIcon) helpIcon.dataset.help = "Enter the amount left over from the previous paycheck. This is added to your income when calculating your Amount Left to Spend.";
        rolloverLabel.childNodes.forEach(n => { if (n.nodeType === 3) n.remove(); });
        rolloverLabel.appendChild(document.createTextNode("Rollover"));
    }
...
function savePaycheckNote(textarea, key) {
 ...
 function renderPaycheckHeader() {
    ...
    const _mk = pc.id;
    const rollover = parseFloat(data.paycheckBudgets?.[_mk]?.rollover || 0);
    ...
    const cashSpent = paycheckBills.filter(b => spendingCats.includes(b.category) && b.paid && !b.creditCard).reduce((s, b) => b.type === "refund" ? s - (parseFloat(getBillDisplayAmount(b)) || 0) : s + (parseFloat(getBillDisplayAmount(b)) || 0), 0);
    ...
    const spendingSegs = spendingCats.map((cat, i) => ({
        ...
        amount: paycheckBills.filter(b => b.category === cat && b.paid && !b.creditCard).reduce((s, b) => b.type === "refund" ? s - (parseFloat(getBillDisplayAmount(b)) || 0) : s + (parseFloat(getBillDisplayAmount(b)) || 0), 0)
    }));
    ...
    const leftHtml = leftLabel ? `<span style="color:var(--mint-text);font-size:12px;white-space:nowrap;"><span class="help-icon" data-help-title="Left to Spend — How it works" data-help="This is the amount of cash available after receiving income, setting aside savings, and paying cash/debit expenses.&lt;br&gt;&lt;br&gt;Formula: Rollover + Income received − Savings − Cash expenses" style="cursor:pointer;margin-right:4px;">📊</span>Left to spend <strong>${formatMoney(leftLabel.amount)}</strong></span>` : "";
...
function savePaycheckRollover(input, mk) {
    const val = parseFloat(input.value) || 0;
    if (!data.paycheckBudgets) data.paycheckBudgets = {};
    if (!data.paycheckBudgets[mk]) data.paycheckBudgets[mk] = {};
    if (val === 0) {
        delete data.paycheckBudgets[mk].rollover;
    } else {
        data.paycheckBudgets[mk].rollover = val;
    }
    saveData();
}
...
window.savePaycheckRollover = savePaycheckRollover;
